...
def test_add_drinks_fail(sys):
    order = sys.create_order()
    try:
        sys.order_product(order, "large Greedy Coke", 2)
    except NotEnoughStockException:
        pass
    else:
        assert False
    assert sys.dict_ingredient_stock["Greedy Coke"][1] == 500
    assert (sys.get_product("large Greedy Coke"), 0) not in order.product_quantities

def test_no_main_type_fail(sys):
    order = sys.create_order()
    try:
        sys.order_product(order, "lettuce slaw", 1)
    except MainTypeNotSelectedException:
        pass
    else:
        assert False

def test_add_minor_filling_fail(sys):
    order = sys.create_order()
    main_type = sys.get_main_type("burger")
    sys.select_main_type(order, main_type)
    try:
        sys.order_product(order, "tomato slice", 10)
    except ExceedLimitException:
        pass
    else:
        assert False
